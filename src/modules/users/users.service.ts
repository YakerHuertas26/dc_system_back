  ...
  async update(id: number, updateUserDto: UpdateUserDto) {
    try {
      const user= await this.findOne(id);
        if (user === updateUserDto) throw new ConflictException('No hay datos para actualizar');

        if(updateUserDto.email){
          const existEmail= await this.userRepository.exists({where:{
            email:updateUserDto.email, userId: Not(id)
          }})
          if (existEmail) throw new ConflictException('El correo electrónico ya está registrado');
        }
